...
class ChatConsole {
  ...
  viewCandidateExplainer() {
    ...
    cy.contains("If you want to know more about how this chat works ", {
      timeout: 30000,
    })
      .contains("click here")
      .as("candidateExplainer")
  ...
  viewCandidateExplainerFromSecondLink() {
    ...
    // as designed to retain chat session while viewing utility pages
    cy.contains(
      "If your candidates experience any issues or require support, they can access our candidate hub or chat to our support team by ",
      { timeout: 30000 }
    )
      .contains("clicking here")
      .as("candidateExplainer")
      .should("have.attr", "target");
    cy.get("@candidateExplainer").invoke("removeAttr", "target").click();
  }
  ...
  viewTermsOfService() {
    // Check target attribute to maintain tabbed behavior of link
    // as designed to retain chat session while viewing utility pages
    cy.contains("Terms of Service", { timeout: 30000 })
      .as("termsOfServiceLink")
      .should("have.attr", "target");
  ...
  viewPrivacyFAQ() {
    // Check target attribute to maintain tabbed behavior of link
    // as designed to retain chat session while viewing utility pages
    cy.contains("Privacy FAQs", { timeout: 30000 })
      .as("privacyFAQLink")
      .should("have.attr", "target");
  ...
  shouldHaveResponse(message) {
    cy.contains('[data-testid="text-bubble"]', message, {
      timeout: 10000,
    }).should("be.visible");
  }
  ...
  shouldHaveMultipleResponse(messages) {
    for (let index = 0; index < messages.length; index++) {
      cy.contains('[data-testid="text-bubble"]', messages[index], {
        timeout: 10000,
      }).should("be.visible");
    }
  }
  ...
  shouldPromptOnEssayLimitsOnLessThan() {
    ...
    cy.get("@essayLimitPrompt")
      .find("p")
      .should("have.text", "You’ve entered less than the recommended 50 words");
  }
  ...
  respondFirstAddressMatchToConsole() {
    cy.get('*[class*="_option-item"]').first().click();
  }
  ...
  selectResponseFromConsole(choice) {
    cy.get(
      `[data-testid="text-bubble"] > div span:contains('${choice}')`
    ).click();
  }
  ...
  askApplicantOnEssayQuestions(userData, botScript) {
    ...
    this.shouldHaveResponse(
      `Thanks for sharing that with us, ${userData.name.split(" ")[0]}.`
    );
    this.shouldHaveResponse(
   ...
   */
  askApplicantOnFirstEssayQuestionThenTryEditFirstResponse(
    userData,
    botScript
  ) {
    this.shouldHaveMultipleResponse(botScript.interviewChallenge1);
    ...
    cy.get(`button[class*="_ellipsis"]`).as("editButton").click();
    cy.interceptRequestEditFreeText(userData.freeText3).as(
      "candidateEditResponseAPI"
    );
    ...
    cy.wait("@candidateEditResponseAPI")
  ...
  askApplicantOnRating(userData, botScript) {
    ...
    this.respondToConsole(userData.freeText1);
    this.shouldHaveResponse(
      `Thank you for your feedback ${userData.name.split(" ")[0]}!`
    );
    this.shouldHaveResponse(
